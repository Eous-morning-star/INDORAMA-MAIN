...
if st.session_state.page == "high_priority_dashboard":
    ...
    if data.empty:
        ...
        # ✅ Check if "High Priority" column exists
        if "High Priority" not in data.columns:
            st.error("⚠️ 'High Priority' column is missing from the dataset.")
        else:
            # ✅ Convert "High Priority" column to boolean
            data["High Priority"] = data["High Priority"].astype(str).str.lower().map({"true": True, "false": False})

            # ✅ Debugging: Show unique values in "High Priority" column
            st.write("🔍 Unique values in 'High Priority':", data["High Priority"].unique())

            # ✅ Filter high-priority equipment
            high_priority_data = data[data["High Priority"] == True]

            if high_priority_data.empty:
                st.info("🚨 No high-priority equipment found in the dataset.")
            else:
                ...
                if filtered_data.empty:
                    st.warning("No records found for the selected date(s).")
                else:
